...
This file is for presenter reference only.
It is intended to stay local and not be pushed to git.

## Bazel
Definition: A build and test system that uses an explicit dependency graph and caching to run only the necessary work.
How to explain: Bazel is a smart planner that avoids rebuilding everything.
Syntax: `bazel <command> <target>`
Example: `bazel run //app:demo_app`

## Target
Definition: A named buildable or testable unit.
How to explain: A target is one unit Bazel knows how to build, run, or test.
Syntax: `//package:target_name`
Example: `//app:demo_app`

## Rule
Definition: The recipe Bazel uses to build a target.
How to explain: A rule is the build type, like a blueprint for how Bazel should treat a target.
Syntax:
```bazel
cc_binary(
    name = "demo_app",
...
```
Example: `cc_binary`, `cc_library`, `cc_test`

## Label
Definition: The unique address of a target.
How to explain: A label is the exact name Bazel uses to find something.
Syntax: `//package:target`
Example: `//tests:math_utils_test`

## Package
Definition: A directory that contains a `BUILD.bazel` file.
How to explain: A package is a folder Bazel treats as one build area.
Syntax: `path/to/package/BUILD.bazel`
Example: `app/`, `lib/`, `tests/`

## BUILD.bazel
Definition: The file where targets and dependencies are declared.
How to explain: This is where we describe what to build and what each thing depends on.
Syntax: `BUILD.bazel`
Example:
```bazel
cc_binary(
...
## MODULE.bazel
Definition: The file used by Bzlmod to define module metadata and external dependencies.
How to explain: This is the entry point for Bazel module dependency management.
Syntax:
```bazel
module(
...
```
Example: `module(name = "bazel_techtalk_demo", version = "1.0")`

## Dependency
Definition: Something a target needs in order to build or run.
How to explain: If app depends on a library, Bazel must build the library first.
Syntax:
```bazel
deps = ["//lib:math_utils"]
```
Example: `//app:demo_app` depends on `//lib:math_utils`

## Dependency Graph
Definition: The full map of targets and their relationships.
How to explain: Bazel works from a graph, not from guesswork.
Syntax: `bazel query //...`
Example: app -> math_utils -> string_utils

## deps(...)
Definition: A query that shows what a target depends on.
Syntax: `bazel query 'deps(//app:demo_app)'`
How to explain: This tells us what the app needs.

## rdeps(...)
Definition: A query that shows what depends on a target.
Syntax: `bazel query 'rdeps(//..., //lib:math_utils)'`
How to explain: This tells us who is affected if this library changes.

## kind(...)
Definition: A query filter that selects targets of a certain kind.
Syntax: `bazel query 'kind(".*test", rdeps(//..., //lib:math_utils))'`
How to explain: This narrows results to just tests.

## Action
Definition: One concrete piece of work Bazel performs, like compile or link.
How to explain: Targets are high-level units; actions are the actual low-level steps.
Syntax: `bazel aquery 'mnemonic("CppCompile", deps(//app:demo_app))'`
Example: compile `main.cc` into an object file

## aquery
Definition: Action query, which shows the actions Bazel will execute.
Syntax: `bazel aquery 'mnemonic("CppCompile", deps(//app:demo_app))'`
How to explain: `query` shows structure; `aquery` shows actual work.

## Mnemonic
Definition: The action type name Bazel assigns to work.
How to explain: A mnemonic is the label for an action type.
Syntax: `mnemonic("CppCompile", <expression>)`
Example: `CppCompile`

## Cache
Definition: Stored outputs Bazel can reuse when inputs are unchanged.
How to explain: If nothing changed, Bazel should not redo the same work.
Syntax: `bazel run //app:demo_app` followed by the same command again
Example: second run reuses cached outputs instead of recompiling

## Incremental Build
Definition: Rebuild only what changed and what depends on it.
How to explain: Change one file, rebuild only the affected path.
Syntax: edit one source file, then rerun `bazel run //app:demo_app`
Example: changing `lib/math_utils.cc` rebuilds the library and app, not everything

## Up-to-date
Definition: Bazel determined the previous result is still valid.
How to explain: Same inputs, same outputs, so Bazel reuses the result.
Syntax: rerun the same Bazel command without changing inputs
Example: second `bazel run //app:demo_app` completes quickly

## Hermetic Build
Definition: A build that depends only on declared inputs.
How to explain: No hidden machine state, no surprise dependencies.
Syntax:
```bazel
cc_library(
    name = "math_utils",
...
```
Example: Bazel rebuilds only from declared source and header inputs

## Deterministic
Definition: Same inputs produce the same outputs every time.
How to explain: Reliable behavior across runs.
Syntax: same target + same inputs = same output
Example: two identical builds produce the same binary result

## Reproducible
Definition: The same build works consistently across machines and environments.
How to explain: Laptop and CI should behave the same way.
Syntax: run the same Bazel target in local and CI environments
Example: `bazel test //tests:math_utils_test` passes both locally and in CI

## Local Cache
Definition: Cached outputs stored on your own machine.
How to explain: The second run is faster because results are already available locally.
Syntax: repeated local runs of the same command
Example: local second run of `bazel run //app:demo_app`

## Remote Cache
Definition: Cached outputs shared across machines.
How to explain: One machine builds it, others reuse it.
Syntax: configure Bazel to use a shared cache endpoint
Example: a teammate reuses CI-generated outputs instead of rebuilding
